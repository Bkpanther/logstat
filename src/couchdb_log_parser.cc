...
Log CouchDBLogParser::ParseLog(const std::vector<std::string> &tokens) const { 
    ...
    const auto level = size > 0 ? log_utils::GetLogLevel(tokens[0]) : log_utils::LogSeverityLevel::UNDEF;
    const std::time_t log_date = size > 1 ? log_utils::GetEpoch(tokens[1], log_parsers::kCouchdbLogDateFormat) : -1;
    if (level != log_utils::LogSeverityLevel::UNDEF && log_date != -1) {
        ...
        const auto method = size > 8 ? log_utils::GetHTTPMethod(tokens[8]) : log_utils::HTTPMethod::UNDEF_ERROR;
        const std::string msg_id = size > 4 ? tokens[4] : log_parsers::kCouchdbNonReqMarker;
        if (msg_id.compare(log_parsers::kCouchdbNonReqMarker) == 0) {
